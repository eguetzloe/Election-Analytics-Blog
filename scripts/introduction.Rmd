...
populationstate <- read.csv("data/datasets_42082_70699_state_pops.csv") %>%
  mutate (year = Year) %>% 
...
abbreviations <- tibble(state = state.name, abb = state.abb)
fulldata <- popvotestate %>%
  left_join(electors, by = c("year", "state"))
#ecdata <- fulldata %>% 
 # left_join(state, by = "state") %>%
 # mutate(ecwinner = ifelse(
 #   R_pv2p > D_pv2p, 1,0)) %>% 
 # mutate(party = ifelse(
  #  ecwinner == 1, "Republican", "Democrat"
 # )) %>%
 # mutate(hover = paste(party, "<br>", "D: ", D, "<br>", "R: ", R, sep = ""))
...
```{r Democratic Vote Share ~ Electors, message=FALSE, warning=FALSE}
...
```{r Southern Strategy, message=FALSE, warning=FALSE}
